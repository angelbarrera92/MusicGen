...
def _do_predictions(texts, melodies, duration, progress=False, **gen_kwargs):
    ...
    for output in outputs:
        with NamedTemporaryFile("wb", suffix=".mp3", delete=False) as file:
            audio_write(
                file.name, output, MODEL.sample_rate, format="mp3", mp3_rate=320, strategy="loudness",
                loudness_headroom_db=16, loudness_compressor=True, add_suffix=False)
            out_mp3s.append(file.name)
...
def predict_full(model, decoder, text, melody, duration, topk, topp, temperature, cfg_coef, progress=gr.Progress()):
    ...
    mp3s = _do_predictions(
        [text], [melody], duration, progress=True,
        top_k=topk, top_p=topp, temperature=temperature, cfg_coef=cfg_coef)
    if USE_DIFFUSION:
        return mp3s[0], mp3s[1]
    return mp3s[0], None, None
    ...
    with gr.Blocks() as interface:
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    duration = gr.Slider(minimum=1, maximum=200, value=10, label="Duration", interactive=True)
                with gr.Row():
            ...
            with gr.Column():
                audio_output = gr.Audio(label="Generated Music (mp3)", type='filepath')
                audio_diffusion = gr.Audio(label="MultiBand Diffusion Decoder (mp3)", type='filepath')
